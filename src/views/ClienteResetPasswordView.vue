    ...
    <div class="bg-white shadow-lg rounded-lg p-8">
      ...
      <form v-if="!passwordReset" @submit.prevent="handleSubmit" class="space-y-4">
        ...
        <button
          ...
          class="w-full px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 transition-colors font-medium disabled:opacity-50 disabled:cursor-not-allowed"
        >
      ...
      >
        ...
        <router-link
          to="/cliente/login"
          class="inline-block mt-4 px-6 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 transition-colors font-medium"
        >
...
onMounted(async () => {
  ...
  // Validar el token
  try {
    const response = await authApiService.validateResetTokenCliente({
      email: email.value,
      token: token.value,
    });
...
const handleSubmit = async () => {
  ...
  try {
    await authApiService.resetPasswordCliente({
      email: email.value,
      token: token.value,
